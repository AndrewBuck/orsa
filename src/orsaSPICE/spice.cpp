...
void SPICE::getPosVel(const std::string & target,
  ...
  spkezr_c(target.c_str(),
	   SPICETime(ephemerisTime), // FromUnits((ephemerisTime-J2000()).asDouble(),Unit::SECOND,-1).get_d(),
...
double SPICE::SPICETime(const orsa::Time & t) {
  ...
  return FromUnits((orsaSolarSystem::ToTimeScale(t,orsaSolarSystem::TS_TDT)-J2000()).asDouble(),Unit::SECOND,-1).get_d();
}
