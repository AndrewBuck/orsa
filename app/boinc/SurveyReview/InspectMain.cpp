...
int main(int argc, char ** argv) {
    ...
    if (argc != 2) {
        ORSA_DEBUG("Usage: %s <sqlite-result-db>",argv[0]);
        exit(0);
    }
...
#warning should read column names for z_* as well, to make sure code works even if column order changes
        ...
        for (int row=1; row<=nrows; ++row) {
            ...
            if (N_NEO>0) {
                ...
                const double NEO_sigma_eta_detect = NEO_sigma_eta_field;
        ...
        for (unsigned int index=0; index<z_H_vec.size(); ++index) {
            ...
            ORSA_DEBUG("NEO at H=%g -- in field: %.2e +/- %.2e (entries: %Zi)   detect: %.2e +/- %.2e (entries: %Zi)   obs: %.2e +/- %.2e",
